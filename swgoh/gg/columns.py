...
class Columns:
    ...
    ICOL.update({"Character": ColumnInfo("Character", "Character", 5)})

    # Primary stats
    ICOL.update({"Primary Stat": ColumnInfo("Primary Stat", "Primary Stat", 6)})
    ICOL.update({"Primary Value": ColumnInfo("Primary Value", "Primary Value", 7)})

    # Secondary stats
    ICOL.update({"Critical Chance": ColumnInfo("Critical Chance %", "Critical Chance", 8)})
    ICOL.update({"Defense": ColumnInfo("Defense", "Defense", 9)})
    ICOL.update({"Defense %": ColumnInfo("Defense %", "Defense", 10)})

    ICOL.update({"Health": ColumnInfo("Health", "Health", 11)})
    ICOL.update({"Health %": ColumnInfo("Health %", "Health", 12)})
    ICOL.update({"Offense": ColumnInfo("Offense", "Offense", 13)})
    ICOL.update({"Offense %": ColumnInfo("Offense %", "Offense", 14)})
    ICOL.update({"Potency": ColumnInfo("Potency %", "Potency", 15)})

    ICOL.update({"Protection": ColumnInfo("Protection", "Protection", 16)})
    ICOL.update({"Protection %": ColumnInfo("Protection %", "Protection %", 17)})
    ICOL.update({"Speed": ColumnInfo("Speed", "Speed", 18)})
    ICOL.update({"Tenacity": ColumnInfo("Tenacity %", "Tenacity", 19)})
