...
def get_mods_selenium(user, password, pct_opt, web_browser="chrome"):
    ...
    swgoh_domain = "swgoh.gg"
    login_res = "/accounts/login/"

    mod_res = "/u/" + swgoh_user + "/mods"

    # Login to swgoh.gg
    try:
        login_url = https + swgoh_domain + login_res
        print("Logging in to swgoh.gg @ %s" % login_url)
        browser.get(login_url)
        user_elm = browser.find_element_by_name("username")
        user_elm.send_keys("boozie")
        pass_elm = browser.find_element_by_name("password")
        pass_elm.send_keys(password)

        submit_btn = browser.find_element_by_class_name("btn-primary")
        submit_btn.click()
        time.sleep(duration)  # sleep for a bit while we login
    ...
    try:
        http = "https://"
        page = "/?page={}"
        mod_url = swgoh_domain + mod_res
        print("Requesting Mods from: %s" % https+mod_url)
        browser.get(https+mod_url+page.format(1))
...
if __name__ == '__main__':
    ...
    parser.add_argument("-w", "--password", required=True, help="SWGOH password")
    ...
    swgoh_pw = args.get('password')
    pct = args.get('pct')

    # SWGOH.GG

    if wb is None:
        rc = get_mods_selenium(swgoh_user, swgoh_pw, pct)
    else:
        rc = get_mods_selenium(swgoh_user, swgoh_pw, pct, wb)
